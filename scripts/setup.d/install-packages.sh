...
for pkg in python3-pip gnupg dmidecode efibootmgr grub2-common grub-pc pigz partclone alsa-utils pulseaudio pulseaudio-utils python3-aiohttp python3-aiohttp-cors; do
    sudo apt install -y --no-install-recommends $pkg
done

if [ x$WCE_TRIAGE_DISK = xtrue ] ; then
    apt remove -y apparmor
    for pkg in xorg openbox aufs-tools xserver-xorg-video-all xserver-xorg-video-vmware vbetool gfxboot python3-aiohttp python3-aiohttp-cors; do
    sudo apt install -y --no-install-recommends $pkg
    done
fi
