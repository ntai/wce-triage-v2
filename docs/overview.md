...
There are two major pieces. One is a HTTP server that runs on the computer  to manipulate the disk. In other word, in order to do any disk manipulation, the HTTP server has to be running. This is written in Python 3 using FastAPI, served by uvicorn.
